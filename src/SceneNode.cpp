...
void SceneNode::Update(glm::mat4 projectionMatrix, Camera* camera, float amplitude, float progress, char baseColor){
    if(m_object!=nullptr){
		...
		if (baseColor == 'R') {
			r = 1.0f - fabs(amplitude);
			g = amplitude * 4.0f;
			b = fabs(amplitude * 10.0f);
		} else if (baseColor == 'B') {
			r = amplitude * 4.0f;
			g = fabs(amplitude * 10.0f);
			b = 1.0f - fabs(amplitude);
		} else {
			r = amplitude * 4.0f;
			g = 1.0f - fabs(amplitude);
			b = fabs(amplitude * 10.0f);
		}

        // Create a 'light'
        m_shader.SetUniform3f("lightColor", r, g, b);
        ...
        m_shader.SetUniform1f("ambientIntensity", 0.8 * (1.0f - progress));
	
		// Iterate through all of the children
		for(int i =0; i < m_children.size(); ++i){
			m_children[i]->Update(projectionMatrix, camera, amplitude, progress, baseColor);
		}
	}
}
